...
export default function Onboarding() {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-[var(--color-bg)] px-4 py-10" id="main-content"><FormShell title="Tell us about your new business" description="We’ll open your Compliance Roadmap with the BIS scheme and steps for this product." onSubmit={buildRoadmap} submitLabel="Build my roadmap"><Field label="Business name" id="new-business-name" value={data.businessName ?? ""} onChange={update("businessName")} /><Field label="Industry" id="new-industry" value={data.industry ?? ""} onChange={update("industry")} /><Field label="Product name" id="new-product-name" value={data.productName ?? ""} onChange={update("productName")} placeholder="e.g. LED bulbs, pressure cookers" required /><SelectField label="Production status" id="production-status" value={data.productionStatus ?? ""} onChange={update("productionStatus")} options={["Prototype", "Pilot", "Ready to manufacture"]} /><div><label htmlFor="made-in" className={labelClass}>Where will it be made?</label><select id="made-in" required value={data.madeIn ?? ""} onChange={(event) => update("madeIn")(event.target.value)} className={inputClass}><option value="">Select an option</option><option value="india">In India</option><option value="abroad">Outside India (imported)</option></select><p className="mt-1 text-xs text-gray-500">Factories outside India certify through a separate BIS scheme, so this changes the route.</p></div><div><label htmlFor="product-description" className={labelClass}>Product description</label><textarea id="product-description" value={data.productDescription ?? ""} onChange={(event) => update("productDescription")(event.target.value)} rows={4} className="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-[var(--color-navy)] focus:outline-none focus:ring-2 focus:ring-[var(--color-navy)]/20" /></div><DocumentsField label="Upload product documents (optional)" hint="Drawings, specifications or any test reports you already have. You can add more later from Documents." /></FormShell></main>;
}
