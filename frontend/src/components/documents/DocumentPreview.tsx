  ...
  if (!document && isLoading) {
    return (
      <main className="flex-1 bg-transparent" id="main-content">
        <p className="mx-auto max-w-xl px-4 py-16 text-sm text-[var(--color-text-muted)]">{t("docv.loadingDoc")}</p>
      </main>
    );
  }
    ...
    return (
      <main className="flex-1 bg-transparent" id="main-content">
        <div className="mx-auto max-w-xl px-4 py-16">
          <EmptyState
            title={t("docv.notFound")}
            description={t("docv.notFoundBody")}
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-navy)]">
                {t(categoryKey(document.category))}
              </p>
              <h1 className="mt-1 break-words text-2xl font-bold text-gray-900">{document.name}</h1>
              <p className="mt-2 text-sm text-gray-500">
                {document.type} · {formatSize(document.size)} · {fill(t("docs.uploadedOn"), { date: formatDate(document.uploadedAt) })}
                {document.expiryDate ? ` · ${fill(t("docs.expiresOn"), { date: formatDate(document.expiryDate) })}` : null}
              </p>
