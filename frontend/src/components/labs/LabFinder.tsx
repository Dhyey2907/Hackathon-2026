...
export default function LabFinder() {
  ...
  const states = ["All locations", ...Array.from(new Set(MOCK_LABS.map((lab) => lab.state)))];
  const scopes = ["All scopes", ...Array.from(new Set(MOCK_LABS.flatMap((lab) => lab.scopes)))].sort();
  const needle = query.trim().toLowerCase();
  const results = MOCK_LABS.filter((lab) => {
    const searchable = [lab.name, lab.address, lab.city, lab.state, ...lab.scopes].join(" ").toLowerCase();
    return (!needle || searchable.includes(needle)) && (state === "All locations" || lab.state === state) && (scope === "All scopes" || lab.scopes.includes(scope));
  });
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[1fr_190px_190px]">
          <label className="relative">
            ...
            <input
              ...
              placeholder="Search cement, Gujarat, Vadodara..."
          ...
          <label>
            ...
            <select
              ...
              className="h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-700 shadow-sm focus:border-[var(--color-navy)] focus:outline-none focus:ring-2 focus:ring-[var(--color-navy)]/20"
            >
              {states.map((item) => <option key={item}>{item}</option>)}
            </select>
          </label>
          <label>
            <span className="sr-only">Filter by test scope</span>
            <select
              value={scope}
          ...
          <article key={lab.id} className="rounded-xl border border-[var(--color-border)] bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">{lab.address}, {lab.state}</p>
              </div>
              <span className="shrink-0 rounded-full bg-green-50 px-2 py-1 text-[11px] font-semibold text-green-700">{lab.status}</span>
            </div>
            <div className="mt-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Recognised scopes</p>
              <div className="mt-2 flex flex-wrap gap-2">
                {lab.scopes.map((item) => (
                  <span key={item} className="rounded-md bg-[var(--color-navy-lighter)] px-2 py-1 text-xs font-medium text-[var(--color-navy)]">
                    {item}
                  </span>
                ))}
              </div>
            ...
            <div className="mt-4 space-y-1 border-t border-gray-100 pt-3 text-sm text-gray-600">
              <p><span className="font-medium text-gray-800">Phone:</span> {lab.phone}</p>
              <p className="break-all"><span className="font-medium text-gray-800">Email:</span> {lab.email}</p>
            </div>
          </article>
        ))}
        {!results.length && (
          <div className="col-span-full">
            <EmptyState
              title="No matching laboratories"
              description="No BIS-recognised testing laboratories match your selected location and test scope."
              actionLabel="Get Started"
              onAction={() => {
                setQuery("");
                setState("All locations");
                setScope("All scopes");
              }}
            />
          </div>
